...
fn tool_settings_path() -> PathBuf { hermes_cli::resolve_hermes_home().join("desktop.json") }

fn read_tool_setting(key: &str) -> String {
...
fn write_tool_setting(key: &str, value: &str) -> Result<(), String> {
    let path = tool_settings_path();
...
pub fn get_tool_paths() -> Result<serde_json::Value, String> {
    Ok(serde_json::json!({
        "git": read_tool_setting("gitPath"),
        "npm": read_tool_setting("npmPath"),
    }))
}
...
pub fn set_tool_paths(git: Option<String>, npm: Option<String>) -> Result<bool, String> {
    if let Some(g) = git { write_tool_setting("gitPath", &g)?; }
    if let Some(n) = npm { write_tool_setting("npmPath", &n)?; }
    Ok(true)
}

pub fn resolve_git_path() -> std::path::PathBuf {
    let custom = read_tool_setting("gitPath");
    if !custom.is_empty() { return std::path::PathBuf::from(custom); }
    // Auto-detect bundled git
    let h = hermes_cli::resolve_hermes_home().join("git");
    for sub in &[["bin", "git.exe"], ["cmd", "git.exe"], ["usr", "bin", "git.exe"]] {
        let p: std::path::PathBuf = sub.iter().fold(h.clone(), |acc, s| acc.join(s));
        if p.exists() { return p; }
    }
    std::path::PathBuf::from("git")
}

pub fn resolve_npm_path() -> std::path::PathBuf {
    let custom = read_tool_setting("npmPath");
    if !custom.is_empty() { return std::path::PathBuf::from(custom); }
