...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await hermesAPI.listCronJobs(true, profile);
      setJobs(list);
    } catch {
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await hermesAPI.removeCronJob(jobId, profile);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      const result =
        job.state === "paused"
          ? await hermesAPI.resumeCronJob(job.id, profile)
          : await hermesAPI.pauseCronJob(job.id, profile);
      if (result.success) {
        await loadJobs();
      } else {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await hermesAPI.triggerCronJob(jobId, profile);
      if (result.success) {
        await loadJobs();
      } else {
