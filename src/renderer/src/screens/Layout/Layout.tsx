...
import {
  ChatBubble, Clock, Users, Settings as SettingsIcon,
  Sparkles, Building, Kanban as KanbanIcon, Download,
...
type View = "chat" | "sessions" | "profiles" | "aiStudio" | "workspace" | "infra" | "settings";
...
function Layout({ verifyWarning, onReinstall, onDismissVerifyWarning }: LayoutProps = {}): React.JSX.Element {
  ...
  const [visitedViews, setVisitedViews] = useState<Set<View>>(() => new Set<View>(["chat"]));
  ...
  useEffect(() => {
    ...
    const b = hermesAPI.onUpdateDownloadProgress((info) => { setDownloadPercent(info.percent); });
    const c = hermesAPI.onUpdateDownloaded(() => { setUpdateState("ready"); setUpdateError(null); });
    const d = hermesAPI.onUpdateError((m) => { setUpdateState("error"); setUpdateError(m); setDownloadPercent(0); });
    return () => { a(); b(); c(); d(); };
  }, []);
  ...
  useEffect(() => {
    const a = hermesAPI.onMenuNewChat(() => {
      hermesAPI.abortChat(); setMessages([]); setCurrentSessionId(null); goTo("chat");
    });
  ...
  async function handleUpdate(): Promise<void> {
    if (updateState === "available" || updateState === "error") {
      ...
      try { const ok = await hermesAPI.downloadUpdate(); if (!ok) setUpdateState("error"); }
      catch (e) { setUpdateError(e instanceof Error ? e.message : String(e)); setUpdateState("error"); }
    } else if (updateState === "ready") { await hermesAPI.installUpdate(); }
  }

  const handleResumeSession = useCallback(async (sessionId: string) => {
    const db = await hermesAPI.getSessionMessages(sessionId);
    const msgs: ChatMessage[] = db.map((m) => ({
      id: `db-${m.id}`, role: m.role === "user" ? "user" : "agent", content: m.content,
      ...(m.attachments?.length ? { attachments: m.attachments } : {}),
    }));
    setMessages(msgs); setCurrentSessionId(sessionId); goTo("chat");
  }, [goTo]);
    ...
    <div className="layout">
      <aside className={`sidebar ${collapsed ? "sidebar-collapsed" : ""}`}>
        ...
        <nav className="sidebar-nav">
          {NAV_ITEMS.map(({ view: v, icon: Icon, labelKey }) => (
            <button key={v} className={`sidebar-nav-item ${view === v ? "active" : ""}`} onClick={() => goTo(v)} title={collapsed ? t(labelKey) : undefined}>
              <Icon size={16} />
      ...
      <main className="content">
        {verifyWarning && onReinstall && onDismissVerifyWarning && (
          <VerifyWarningBanner onReinstall={onReinstall} onDismiss={onDismissVerifyWarning} />
        )}

        <div style={paneStyle("chat")}>
          <Chat messages={messages} setMessages={setMessages} sessionId={currentSessionId} profile={activeProfile}
            onNewChat={() => { hermesAPI.abortChat(); setMessages([]); setCurrentSessionId(null); }} />
        </div>
        ...
        {visitedViews.has("aiStudio") && <div style={paneStyle("aiStudio")}>
          <TabPage tabs={[
            { key: "models", label: t("navigation.models") }, { key: "providers", label: t("navigation.providers") },
            { key: "skills", label: t("navigation.skills") }, { key: "persona", label: t("navigation.persona") },
            { key: "tools", label: t("navigation.tools") }, { key: "memory", label: t("navigation.memory") },
        ...
        {visitedViews.has("workspace") && <div style={paneStyle("workspace")}>
          <TabPage tabs={[
        ...
        {visitedViews.has("infra") && <div style={paneStyle("infra")}>
          <TabPage tabs={[
            ...
            {(tab) => {
              switch (tab) {
                case "gateway": return <Gateway profile={activeProfile} />;
                case "mcp": return <McpServers />;
                case "plugins": return <Plugins />;
                case "curator": return <Curator />;
                case "schedules": return <Schedules profile={activeProfile} />;
                default: return null;
              }
            }}
          </TabPage>
        ...
        {visitedViews.has("settings") && <div style={paneStyle("settings")}>
          <TabPage tabs={[
            ...
            {(tab) => {
              switch (tab) {
                case "general": return <Settings profile={activeProfile} />;
