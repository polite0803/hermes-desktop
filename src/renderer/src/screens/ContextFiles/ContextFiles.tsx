...
export default function ContextFiles(): React.JSX.Element {
  ...
  const load = useCallback(async () => {
    try { setFiles(await hermesAPI.listContextFiles()); } catch {}
    setLoading(false);
  }, []);
  ...
  if (loading) return <div className="settings-container"><h1 className="settings-header">{t("navigation.contextFiles")}</h1><div className="loading-spinner" /></div>;

  return (
    <div className="settings-container" style={{ flexDirection: "row", gap: 0 }}>
      <div style={{ width: 200, borderRight: "1px solid var(--border)", padding: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <h2 style={{ fontSize: 14, fontWeight: 600 }}>{t("navigation.contextFiles")}</h2>
          <button className="btn btn-sm" onClick={handleCreate}><Plus size={12} /></button>
        </div>
        {files.map((f) => (
          <div key={f} className={`sidebar-nav-item ${selected === f ? "active" : ""}`}
      ...
      <div style={{ flex: 1, padding: 12 }}>
        {selected ? (
          ...
          <div className="models-empty"><p className="models-empty-text">{t("contextFiles.selectFile")}</p></div>
        )}
      </div>
    </div>
  );
